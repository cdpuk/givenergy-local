...
class Framer(ABC):
    ...
    * ``crc`` (2 bytes) CRC for a request is calculated using the function id, base register and
    ...
    async def decode(self, data: bytes) -> AsyncIterator[Union[BasePDU, ExceptionBase]]:
        ...
        while (
            len(self._buffer) >= 18
        ):  # shortest known message is 18b (heartbeat request)
            ...
            _logger.debug(
                f"Found next frame: 0x{self._buffer[:8].hex()}..., buffer_len={len(self._buffer)}"
            )
            ...
            hdr_len, u_id, f_id = (
                int.from_bytes(self._buffer[4:6], byteorder="big"),
                self._buffer[6],
                self._buffer[7],
            )
