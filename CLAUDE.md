...
- `coordinator.py` — `GivEnergyUpdateCoordinator` (a `DataUpdateCoordinator[Plant]`) is the heart of the integration. It polls every 10s, forces a full refresh every 5 min, retries on bad data, and reconnects on timeout. **`_is_data_valid()` + `_INVERTER_QUALITY_CHECKS`** discard implausible register values (a real problem that otherwise corrupts the Energy Dashboard). `execute()` runs write commands then forces a refresh.
